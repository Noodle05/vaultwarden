...
match (table, operation) {
    ...
    ("users", "update")            => {
        let logout_cols = ["password_hash", "security_stamp", "client_kdf_type",
                           "client_kdf_iter", "email", "api_key", "enabled"];
...
## Comparison with ha-websocket-design.md

The two designs agree on the core architecture (trigger → sidecar → internal API). Differences resolved:

| Aspect | Final decision | Reason |
|--------|---------------|--------|
| Tables | 10 (not 9, not 14) | Added `archives`; removed `users_collections`, `groups`, `groups_users`, `collections_groups` after verifying no WS is sent |
| Payload | Include `old`/`new` + `changed_columns` | `old` needed for DELETE (row gone); `changed_columns` for clean user notification logic |
| Trigger function | Single generic `notify_table_change()` | Cleaner than per-table functions |
| Replay methods | WS-only `replay_*` helpers | Avoids duplicate push notifications |
| Bulk operations | Per-cipher for v1 | Simpler, clients handle gracefully |
| users_organizations | INSERT + UPDATE + DELETE | INSERT = new member (need SyncOrgKeys on confirm) |
